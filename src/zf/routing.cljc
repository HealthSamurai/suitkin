...
(rf/reg-event-fx
 :fragment-changed
 (fn [{db :db} [k fragment]]
   ;; FIXME what ? ;;NOTE ofc we copied this too
...
(rf/reg-fx
 ...
 (fn [params]
   (let [opts  (parse-fragment #?(:cljs (.. js/window -location -hash)
                                  :clj  {}))]
     #?(:cljs (aset (.. js/window -location) "hash" (to-hash (assoc opts :params params)))))))
